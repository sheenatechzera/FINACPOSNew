...
namespace FinacPOS
{
    public partial class frmPOSPaymentVoucher : Form
    {
        public frmPOSPaymentVoucher()
        {
            InitializeComponent();
        ...
        POSPaymentDetailsSP POSpaymentDetailsSP = new POSPaymentDetailsSP(); 
        ...
        public string POSBillNumberMax()
        {
            ...
            {
                DataTable dtbl = new DataTable();
                dtbl = SPGeneral.GetPOSLastBillNo(PublicVariables._counterId, "Payment");
                ...
                PartBillNo = "PV" + PublicVariables._counterId + "" + DateTime.Now.Date.ToString("yy") + "" + PartBillNo;
            }
            catch (Exception ex)
            {
        ...
        public void FormLoadFunction()
        {
            ...
            if (DateTime.Compare(Convert.ToDateTime(DateTime.Today), Convert.ToDateTime(strSessionDate)) > 0)
            {
                ...
                {
                    MessageBox.Show("Opened Session Date is not Today's Date", "", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else if (PublicVariables._ModuleLanguage == "ARB")
                {
                    MessageBox.Show("تاريخ الجلسة المفتوحة ليس تاريخ اليوم", "", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
        ...
        private void frmPOSPaymentVoucher_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        public bool CheckGrid()
        {
            ...
            try
            {
                if (txtCustomerId.Text == "")
                {
                    ...
                    if (PublicVariables._ModuleLanguage == "ENG")
                    {
                        MessageBox.Show("Select Ledger", "Information",
                            MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else if (PublicVariables._ModuleLanguage == "ARB")
                    {
                        MessageBox.Show("اختر الحساب", "معلومة",
                            MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                ...
                //else if (cmbCashOrBank.SelectedValue == null) //blocked on 29/mar/2025 for ledger load from counter master
                //{
                //    isOk = false;
                ...
                else if (txtTotalAmount.Text == "0" || txtTotalAmount.Text == "0.00" || txtTotalAmount.Text == "")
                {
                    ...
                    if (PublicVariables._ModuleLanguage == "ENG")
                    {
                        MessageBox.Show("Amount should not be zero", "Information",
                            MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else if (PublicVariables._ModuleLanguage == "ARB")
                    {
                        MessageBox.Show("يجب ألا يكون المبلغ صفرًا", "معلومة",
                            MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                ...
                else
                {
                    ...
                    if (dgvPartyBalance.Rows.Count > 1)
                    {
                        ...
                        else
                        {
                            if (PublicVariables._ModuleLanguage == "ENG")
                            {
                                MessageBox.Show("Can't save party balance without at least one row with complete details", "Information",
                                    MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                            else if (PublicVariables._ModuleLanguage == "ARB")
                            {
                                MessageBox.Show("لا يمكن حفظ رصيد الطرف إلا بوجود صف واحد على الأقل مكتمل التفاصيل", "معلومة",
                                    MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                                                    ...
                                                    if (dgvRow.Cells["type"].Value.ToString() == "Against")
                                                    {
                                                        ...
                                                        if (decimal.Parse(dgvRow.Cells["amountToPay"].Value.ToString()) < 0)// for salesReturn
                                                        {
                                                            if (decimal.Parse(dgvRow.Cells["amount"].Value.ToString()) < decimal.Parse(strAmountToPay[0]))
                                                            {
                                                                ...
                                                                if (PublicVariables._ModuleLanguage == "ENG")
                                                                {
                                                                    MessageBox.Show("Amount exceeds the amount to pay", "Information",
                                                                        MessageBoxButtons.OK, MessageBoxIcon.Information);
                                                                }
                                                                else if (PublicVariables._ModuleLanguage == "ARB")
                                                                {
                                                                    MessageBox.Show("المبلغ يتجاوز المبلغ المطلوب سداده", "معلومة",
                                                                        MessageBoxButtons.OK, MessageBoxIcon.Information);
                                                                }
        ...
        public void  SaveFunction()
        {
            ...
            try
            {
                ...
                POSPaymentMasterInfo InfoPOSPaymentMaster = new POSPaymentMasterInfo();
                InfoPOSPaymentMaster.paymentNo = lblBillNo.Text;
                InfoPOSPaymentMaster.billDate = Convert.ToDateTime(lblBillDate.Text);
                InfoPOSPaymentMaster.billTime = lblBillTime.Text;
                InfoPOSPaymentMaster.sessionDate = Convert.ToDateTime(lblSessionDate.Text);
                InfoPOSPaymentMaster.counterId = lblCounter.Text;
                InfoPOSPaymentMaster.sessionNo = lblSessionNO.Text;

                if (RbtnCash.Checked == true) //ADDED on 29-03-2025 by nishana
                {
                    InfoPOSPaymentMaster.CashOrBankledgerId = counterInfo.CashAccountLedgerId;
                    InfoPOSPaymentMaster.PaymentType =  RbtnCash.Text;
                }
                else if (RbtnUPI.Checked == true)
                {
                    InfoPOSPaymentMaster.CashOrBankledgerId = counterInfo.UPIAccountLedgerId;
                    InfoPOSPaymentMaster.PaymentType = RbtnUPI.Text;
                }
                else  if (RbtnCreditCard.Checked == true)
                {
                    InfoPOSPaymentMaster.CashOrBankledgerId = counterInfo.BankAccountLedgerId;
                    InfoPOSPaymentMaster.PaymentType = RbtnCreditCard.Text;

                }

              //  InfoPOSPaymentMaster.CashOrBankledgerId = cmbCashOrBank.SelectedValue.ToString(); blocked on 29-03-2025
                InfoPOSPaymentMaster.narration = txtNarration.Text;
                InfoPOSPaymentMaster.userId = PublicVariables._currentUserId;
                InfoPOSPaymentMaster.totalAmount = decimal.Parse(txtTotalAmount.Text);
                InfoPOSPaymentMaster.extra1 = "";
                if (rbtnGeneral.Checked)
                    InfoPOSPaymentMaster.LedgerType = "General";
                else if(rbtnVendor.Checked)
                    InfoPOSPaymentMaster.LedgerType = "Vendor";
                if (!isInEditMode)
                {
                    strMasterId = POSpaymentMasterSP.POSPaymentMasterAdd(InfoPOSPaymentMaster);
                }
                else
                {
                    InfoPOSPaymentMaster.POSPaymentMasterId = strMasterIdForEdit;
                    POSpaymentMasterSP.POSPaymentMasterEdit(InfoPOSPaymentMaster);
                }
                //if (strMasterId != "")
                //{
                POSPaymentDetailsInfo InfoPOSPaymentDetails1 = new POSPaymentDetailsInfo();
                    if (isInEditMode)
                    {
                        InfoPOSPaymentDetails1.POSPaymentMasterId = strMasterIdForEdit;
                    }
                    else
                        InfoPOSPaymentDetails1.POSPaymentMasterId = strMasterId;
                    InfoPOSPaymentDetails1.ledgerId = lblLedgerId.Text;
                    InfoPOSPaymentDetails1.amount = Convert.ToDecimal(txtTotalAmount.Text);
                    InfoPOSPaymentDetails1.chequeNo = txtChequeNo.Text;
                    InfoPOSPaymentDetails1.chequeDate = DateTime.Now;
                    InfoPOSPaymentDetails1.extra1 = "";
                    string strPODetailsId = "";
                    if (!isInEditMode)
                    {
                        strPODetailsId = POSpaymentDetailsSP.POSPaymentDetailsAdd(InfoPOSPaymentDetails1);
                    }
                    else
                    {
                        InfoPOSPaymentDetails1.POSPaymentDetailsId = strDetailsIdForEdit;
                        POSpaymentDetailsSP.POSPaymentDetailsEdit(InfoPOSPaymentDetails1);
                    }
                    if (isInEditMode)
                    {
                      ...
                      //  IntrestParameterSettingsSP SpInterest = new IntrestParameterSettingsSP();
                        SpPosting.LedgerPostingDeleteByVoucherTypeAndVoucherNo(InfoPOSPaymentDetails1.POSPaymentMasterId, "POSPayment");
                        SpPartyBalance.PartyBalanceDeleteByVoucherTypeVoucherNoAndReferenceType(InfoPOSPaymentDetails1.POSPaymentMasterId, "POSPayment");

                    }
                    // Add details in grid to ledger posting 
                    DoLedgerPosting(InfoPOSPaymentDetails1.ledgerId, InfoPOSPaymentDetails1.amount, InfoPOSPaymentDetails1.POSPaymentMasterId, true);
                    ...
                    if (Info.BillByBill && BillByBill)
                    {
                        ...
                        DoPartyBalance(InfoPOSPaymentDetails1.ledgerId, InfoPOSPaymentDetails1.POSPaymentMasterId);
                    }
                   ...
                   // DoInterestparameterSettings(InfoPOSSalesDetails1.ledgerId, InfoPOSSalesDetails1.POSReceiptMasterId);
                    ...
                    DoLedgerPosting(InfoPOSPaymentMaster.CashOrBankledgerId, InfoPOSPaymentMaster.totalAmount, InfoPOSPaymentDetails1.POSPaymentMasterId, false);
               // }
                if (!isInEditMode)
                {
                    spGeneral.UserActivityAdd("POSPayment", "Save", InfoPOSPaymentDetails1.POSPaymentMasterId, lblBillNo.Text, Convert.ToDateTime(lblBillDate.Text), decimal.Parse(txtTotalAmount.Text), decimal.Parse(txtTotalAmount.Text));
                    ...
                    //}
                        ...
                        SPGeneral.POSBillUpdate(PublicVariables._counterId, PublicVariables._currentUserId, "Payment");
                ...
                {
                    spGeneral.UserActivityAdd("POSPayment", "Edit", InfoPOSPaymentDetails1.POSPaymentMasterId, lblBillNo.Text, Convert.ToDateTime(lblBillDate.Text), decOldAmount, decimal.Parse(txtTotalAmount.Text));
        ...
        public void DoLedgerPosting(string strLedgerId, decimal dcAmount, string voucherNo, bool isMaster)
        {
            try
            {
                ...
                InfoPosting.VoucherType = "POSPayment";
                InfoPosting.postingType = "POSPayment";
                                    ...
                                    if (isSelect)
                                    {
                                        ...
                                        {                                           
                                            ...
                                            if (dRow.Cells["type"].Value.ToString() == "Against")
                                            {
                                                ...
                                                InfoPatryBalance.AgainstVoucherType = "POSPayment";
                                            ...
                                            {
                                                InfoPatryBalance.VoucherType = "POSPayment";
                                            ...
                                            InfoPatryBalance.Debit = decimal.Parse(dRow.Cells["amount"].Value.ToString());
        ...
        private void FillGrid(bool IsFromLoadPEnding)
        {
            try
            {
                ...
                {
                    dtblPartyBalance = spPartyBalance.PartyBalanceViewByLedgerIdNew(strLedgerId, "Dr");
                }
                    ...
                    for (int i = 0; i < dtblPartyBalance.Rows.Count; ++i)
                    {
                       ...
                       // dgvPartyBalance.Rows[dgvPartyBalance.Rows.Count - 2].Cells["BillAmount"].Value = Math.Round((decimal.Parse(dtblPartyBalance.Rows[i]["BillAmount"].ToString()) / InfoCurrencyConversion.Rate), 2);
                        ...
                        if (strMasterIdForEdit != "" && !IsFromLoadPEnding)
                        {
                            ...
                            dtblAgainst = spPartyBalance.PartyBalanceGetByAgainstNew("POSPayment", strMasterIdForEdit, strLedgerId, "Dr");
                        ...
                        dgvPartyBalance.Rows[dgvPartyBalance.Rows.Count - 2].Cells["crOrDr"].Value = "Dr";// Dr for payment and Cr for Receipt
                        isFormLoad = true;
                ...
                if (e.RowIndex != -1 && e.ColumnIndex != -1)
                {
                    if (isWorkValueChanged)
                    {
                        ...
                        else if (e.ColumnIndex == dgvPartyBalance.Columns["type"].Index && e.RowIndex >= 0)
                        {
                            ...
                            {
                                isWorkValueChanged = false;
                                dgvPartyBalance.Rows[e.RowIndex].Cells["crOrDr"].Value = "Dr";
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            frmlookup.strSearchColumn = "paymentNo";
            frmlookup.strSearchOrder = " POSPaymentMasterId DESC ";
            frmlookup.strSearchQry = " POSPaymentMasterId,paymentNo,Date,BankCash,Narration,totalAmount ";
            string strdate = "";
            ...
            frmlookup.DoWhenComingFromPaymentForm(this);
        }
        ...
        public void FillPayment()
        {
            try
            {
                ...
                POSPaymentMasterSP SpMaster = new POSPaymentMasterSP();
                POSPaymentDetailsSP SpDetails = new POSPaymentDetailsSP();
                POSPaymentMasterInfo InfoMaster = new POSPaymentMasterInfo();
               
                PartyBalanceSP SpBalance = new PartyBalanceSP();
                CurrencyConversionSP SpCurrencyConversion = new CurrencyConversionSP();
                DataTable dtMaster = SpMaster.POSPaymentMasterViewByMasterId(strMasterIdForEdit);
                if(dtMaster.Rows.Count>0)
                {
                    strMasterIdForEdit = dtMaster.Rows[0]["POSPaymentMasterId"].ToString();
                    lblBillNo.Text = dtMaster.Rows[0]["paymentNo"].ToString();
                    ...
                    DataTable dtDetails = SpDetails.POSPaymentDetailsViewByMasterId(strMasterIdForEdit);
                    if (dtDetails.Rows.Count > 0)
                    {
                        strDetailsIdForEdit = dtDetails.Rows[0]["POSPaymentDetailsId"].ToString();
        ...
        public void FillPartyBalanceDatatable()
        {
            try
            {
                ...
                DataTable dtbl = SpPartyBanace.PartyBalanceByVoucherTypeAndVoucherNoNew("POSPayment", strMasterIdForEdit, "Dr");
                ...
                if (dtbl.Rows.Count > 0)
                {
                    for (int i = 0; i < dtbl.Rows.Count; i++)
                    {
                       ...
                       // nrow["LedgerId"] = dtbl.Rows[i]["ledgerId"].ToString();
                        ...
                        nrow["amount"] = Math.Round((decimal.Parse(dtbl.Rows[i]["debit"].ToString()) / InfoCurrencyConversion.Rate), 2);
        ...
        private void btnLoadPending_Click(object sender, EventArgs e)
        {
            try
            {
                if (lblLedgerId.Text != "")
                {
                    dtblParty = spPartyBalance.PartyBalanceViewByLedgerIdNew(lblLedgerId.Text, "Dr");
                    if (dtblParty.Rows.Count > 0)
                        FillGrid(true);
                    else
                        if (PublicVariables._ModuleLanguage == "ENG")
                        ...
                        if (MessageBox.Show(clsGeneral.MessageFunction("ConfirmDelete"), "", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == DialogResult.Yes)
                        {
                            ...
                            if (dtblParty.Rows.Count > 0)
                            {
                                ...
                                foreach (DataRow dRow in dtblParty.Rows)
                                {
                                    ...
                                    {
                                        isExistParty = SpBalance.PartyBalanceReferenceCheck(strMasterIdForEdit, "POSPayment");
                                        if (isExistParty)
                                            break;
                                    }
                                }
                            ...
                            if (!isExistInterest && !isExistParty)
                            {
                                ...
                                SpMaster.POSPaymentVoucherDeleteDetails(strMasterIdForEdit);
                                spGeneral.UserActivityAdd("POSPayment", "Delete", strMasterIdForEdit, lblBillNo.Text, Convert.ToDateTime(lblBillDate.Text), decimal.Parse(txtTotalAmount.Text), decimal.Parse(txtTotalAmount.Text));
        ...
        public void FillDatatatablesforDevPrint(bool isDuplicatePrint, string strDuplicateBillNo)
        {
            ...
            if (isDuplicatePrint == false)
            {
                ...
                dRowDetails["PaymentNo"] = lblBillNo.Text;
                ...
                else if (RbtnUPI.Checked)
                {
                    paymentMode = RbtnUPI.Text;
                }
                else if (RbtnCreditCard.Checked)
                {
                    paymentMode = RbtnCreditCard.Text;
                }
                dRowDetails["PaymentMode"] = paymentMode;
        ...
        private void txtCustomerId_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                MessageBox.Show("PV50:" + ex.Message, "", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
