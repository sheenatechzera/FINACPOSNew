...
namespace FinacPOS
{
    class ExportNew
    {
        public bool CheckWhetherOfficeInstalled()
        {
            ...
            if (TargetKey == null)
            {
                MessageBox.Show("Install Office", "", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return false;
        ...
        public void ExportExcel(DataGridView dgv, string rptName, int inFirstRow, int inFirstCol, string Format, object dtFromDate, object dtToDate, string header)//, string credit, string debit, string closing)
        {
            try
            {
                if (CheckWhetherOfficeInstalled())
                {
                    ...
                    range.Value2 = PublicVariables._currentDate.Date.ToString("dd-MMM-yyyy");
                    range.Font.Bold = true;
        ...
        public void ExportExcelDtbl(DataTable dgv, string rptName, int inFirstRow, int inFirstCol, string Format, object dtFromDate, object dtToDate, string header)//, string credit, string debit, string closing)
        {
            try
            {
                if (CheckWhetherOfficeInstalled())
                            ...
                            for (int inCol = inFirstCol; inCol < dgv.Columns.Count; inCol++)
                            {
                                if (inRow == 0)
                                {
                                    ...
                                    //{
                                        ...
                                        range.Font.Bold = true;
                                        range.Interior.Color = ColorTranslator.ToWin32(Color.LightGray);
                                        range.Value2 = dgv.Columns[inCol].ColumnName;
                                    //}
                                }
                                range = (Excel.Range)ws.Cells[inNewRow + 9, inColN];
                                ...
                                //{
                                    ...
                                    range.BorderAround(Excel.XlLineStyle.xlContinuous, Excel.XlBorderWeight.xlHairline, Excel.XlColorIndex.xlColorIndexAutomatic, 1);
                                    if (dgv.Rows[inRow][inCol].ToString() != null)
                                    {
                                        string str = dgv.Rows[inRow][inCol].ToString();

                                        try
                                        {
                                            if (dgv.Columns[inCol].ToString().Replace(" ", "").ToLower() == "phoneno" || dgv.Columns[inCol].ToString().Replace(" ", "").ToLower() == "phonenumber")
                                                range.NumberFormat = "@";
                                            else
                                            {
                                                decimal.Parse(str);
                                                decimal dc = Math.Round(decimal.Parse(str), 2);
                                                str = dc.ToString();
                                                if (dgv.Columns[inCol].ToString().ToLower() == "debit" || dgv.Columns[inCol].ToString().ToLower() == "credit")
                                                    range.NumberFormat = "#00.00#";
                                                else
                                                    range.NumberFormat = "General";
                                            }
