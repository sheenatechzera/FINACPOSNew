...
namespace FinacPOS
{
    public partial class rptPOSExchangedItem : Form
    {
        public rptPOSExchangedItem()
        {
            InitializeComponent();
        }
        #region DECLARATIONS
        ...
        #endregion
        private void rptPOSExchangedItem_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void rptPOSExchangedItem_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void Print()
        {
            ...
            string reportnamepart = "Exchanged Item Report";
            ...
            else
            {
                if (rbtnBillDate.Checked == true)
                {
                    ...
                    dtblReport = objPOSReportSP.POSExchagedItemReport(Convert.ToDateTime(dtpFrom.Text), Convert.ToDateTime(dtpTo.Text), "BILLDATE");
                }
                else if (rbtnSessionDate.Checked == true)
                {
                    strReportName = reportnamepart + " (Session Date)";
                    dtblReport = objPOSReportSP.POSExchagedItemReport(Convert.ToDateTime(dtpFrom.Text), Convert.ToDateTime(dtpTo.Text), "SESSIONDATE");
                }
            ...
            if (dtblReport.Rows.Count > 0)
            {
                ...
                if (strFormat != "Print")
                {
                    ...
                    frmreportviewobj.POSExchangedItemReportPrinting(dtFinal, dtblCompanyDetails, strReportName, strDatePeriod);
